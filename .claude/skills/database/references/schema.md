# Initial Schema — full SQL (spec §14)

The canonical starting schema for "מזל טוב AI". Apply as the first migrations;
later migrations evolve from here. RLS policy *content* is defined by the
`access-control` skill — this file only enables RLS. Money is integer agorot.
V2–V3 entities (spec §14: `organizations`, `fonts`, `colors`, `layouts`,
`favorites`, `reviews`, `affiliates`, `affiliate_links`) are intentionally
omitted until their phase (spec §15).
...
-- coupons: discount codes (spec §9, §11 admin-managed)
...
## Dashboard views (spec §10)

Metrics and the conversion funnel come from SQL, never from client-side counting.

```sql
-- Revenue and order counts per day
...
The funnel (visited → started filling → reached payment → paid → abandoned,
spec §10) is derived from PostHog events joined with `documents`/`orders`
timestamps in a dedicated RPC — add it when the dashboard is built.

Later phases (spec §14–15): `organizations`, `fonts`, `colors`, `layouts`,
`favorites`, `reviews`, `affiliates`, `affiliate_links` — add via new migrations
when V2/V3 begins, not up front.
